...
onEvent("btn_crearRegistro", "click", function() {
  ...
  //PRUEBA DE SET DE VALOR
 /* setKeyValue("registro", rec, function () {
    console.log("creado");
  });*/
  
  var bandera = false;
  
  createRecord("Mediciones", rec,function(record) {
  ...
  if (bandera = true) {
    ...
    setProperty("btn_crearRegistro", "background-color", "blue");
  } else {
    console.log("no se grabo");
  setProperty("btn_crearRegistro", "background-color", "red");
  }
});
...
/*voy a crear un array de objetos en donde cada objeto va a ser un relevamiento con datos.
voy a ir almacenando esos datos en el array y que si hay internet que se suban a la DB y sino que se 
queden en ese array hasta que se puedan subir a internet.
deberia haber una pantalla que muestre cuales son las muestras que no fueron subidas, con un boton 
para "sincronizar" los datos entre la BD y la app.
O incluso lo mejor seria que se pueda hacer de forma automatica.
*/

/*DESARROLLO DE SINCRO AUTOMATICO

*/


/*cuando hago click en el boton guardar save genero un objeto que guarde todo
el record
*/
/*onEvent("btn_crearRegistro", "click", function( ) {
  var rec =  crearObjeto();
...
//evento para ir a la pantalla de sincro
onEvent("btnPantallaSincro", "click", function( ) {
  getKeyValue("registro", function (value) {
    createRecord("Mediciones", value,function(record) {
      showElement("lbl_RegistroCreado");
      setTimeout(function() {
      hideElement("lbl_RegistroCreado");
      setScreen("ScreenINICIO");
  ...
  setScreen("scrSincronizacion");
});

//evento para ir a la pantalla de INICIO
onEvent("btnIrAMenuDesdeSincro", "click", function( ) {
  setScreen("ScreenINICIO");
});
...
function crearObjeto() {
  var objeto = {};
  objeto.Fecha                   = getText("text_inputFecha");
  objeto.Lote                    = getText("text_inputLOte");
  objeto.Recurso                 = getText("text_inputRecurso");
  objeto.superficieParcela       = getText("text_inputSupparcela");
  objeto.porcentajeMateriaSeca   = getText("text_inputPorcMS");
  objeto.numVacas                = getText("text_inputNrovaca");
  objeto.KgMSDisponible          = getText("lbl_ResMSdisp");
  objeto.KgMSRemanente           = getText("lbl_ResMSrem");
  objeto.ConsumoMSHa             = getText("lbl_ResConsumoMsha");
  objeto.ConsumoAnimal           = getText("lbl_ResConsumo");
  objeto.PorcAprovechamiento     = getText("lbl_ResAprovechamiento");
  objeto.MetroCuadradosporanimal = getText("lbl_m2Animal");
